...
<Dialog.Root bind:open={unlinkTOTPSecondStepModalOpen}>
  <Dialog.Content>
    <Dialog.Header>
      <Dialog.Title>{$t('auth.totp.settings.manage.title')}</Dialog.Title>
    </Dialog.Header>
...
<Dialog.Root bind:open={setUpTOTPModalOpen}>
  <Dialog.Content>
    <Dialog.Header>
      ...
      <Dialog.Description>
        {#if user.totpSecret}
          {$t('auth.totp.settings.manage.description')}
        {:else}
          {$t('auth.totp.settings.setup.description')}
        {/if}
      </Dialog.Description>
    </Dialog.Header>
    ...
    {#if user.totpSecret}
      <Dialog.Footer>
        <Dialog.Close type="button" variant="outline" onclick={() => (setUpTOTPModalOpen = false)}
          >{$t('auth.totp.settings.success.buttons.close')}</Dialog.Close
        >
        <Button
          type="submit"
          onclick={() => {
    ...
    {:else}
      ...
      <div class="flex flex-col items-center justify-center gap-4">
        ...
        {:then url}
          <img src={url} alt={$t('auth.totp.settings.setup.qrCodeAlt')} class="mx-auto size-64" />
...
<Dialog.Root bind:open={TOTPSuccessModalOpen}>
  <Dialog.Content>
    <Dialog.Header>
      <Dialog.Title>{$t('auth.totp.settings.success.title')}</Dialog.Title>
      <Dialog.Description>
        {$t('auth.totp.settings.success.description')}
      </Dialog.Description>
    </Dialog.Header>
...
{#if user.totpSecret}
  ...
  <Button onclick={() => (setUpTOTPModalOpen = true)}
    >{$t('auth.totp.settings.page.disable.button')}</Button
  >
{:else}
  <p>{$t('auth.totp.settings.page.enable.description')}</p>
  <Button onclick={() => (setUpTOTPModalOpen = true)}
    >{$t('auth.totp.settings.page.enable.button')}</Button
  >
{/if}

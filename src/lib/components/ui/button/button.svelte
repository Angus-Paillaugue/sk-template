  ...
  export const buttonVariants = tv({
    ...
    variants: {
      variant: {
        ...
        none: 'bg-transparent hover:bg-transparent dark:bg-transparent dark:hover:bg-transparent text-foreground',
      },
      size: {
...
{#snippet textRow(text: string, className?: string)}
  <span class={cn('z-10', className)}>
    {#each text.split('') as l, i}
      <span
        class="ease-elastic-out pointer-events-none inline-block leading-6 motion-safe:group-hover/button:animate-[text-appear_700ms_var(--ease-elastic-out)_forwards]"
